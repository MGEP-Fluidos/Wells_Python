...
class GA:
    def __init__(self, 
        ...
        if callable(fitness_func):
            # Check if the fitness function accepts 7 paramaters.
            if (fitness_func.__code__.co_argcount == 7):
                self.fitness_func = fitness_func
            else:
                self.valid_parameters = False
                raise ValueError("The fitness function must accept 7 parameters representing the GA instance, the solution to which the fitness value is calculated and the solution index within the population.\nThe passed fitness function named '{funcname}' accepts {argcount} argument(s).".format(funcname=fitness_func.__code__.co_name, argcount=fitness_func.__code__.co_argcount))
        else:
            self.valid_parameters = False
        ...
        if not (on_start is None):
            ...
            if callable(on_start):
                ...
                if (on_start.__code__.co_argcount == 6):
                    self.on_start = on_start
                else:
                    self.valid_parameters = False
                    raise ValueError("The function assigned to the on_start parameter must accept 6 parameters representing: the instance of the genetic algorithm, the 'par_to_fit' string, the 'writepops' boolean,the 'rootfilename' string, the parameters to be written as headers in the output file and the running mode of the genetic algorithm.\nThe passed function named '{funcname}' accepts {argcount} argument(s).".format(funcname=on_start.__code__.co_name, argcount=on_start.__code__.co_argcount))
            else:
                self.valid_parameters = False
        ...
        if not (on_fitness is None):
            ...
            if callable(on_fitness):
                ...
                if (on_fitness.__code__.co_argcount == 2):
                    ...
                    self.valid_parameters = FalseRayuela
        ...
        if not (on_generation is None):
            ...
            if callable(on_generation):
                # Check if the on_generation function accepts 2 paramaters.
                if (on_generation.__code__.co_argcount == 3):
                    self.on_generation = on_generation
                else:
                    self.valid_parameters = False
                    raise ValueError("The function assigned to the on_generation parameter must accept only 3 parameters representing the instance of the genetic algorithm, the parameters to be written as headers in the output file and the mode in which the algorithm is run.\nThe passed function named '{funcname}' accepts {argcount} argument(s).".format(funcname=on_generation.__code__.co_name, argcount=on_generation.__code__.co_argcount))
            else:
                self.valid_parameters = False
        ...
        if not (on_stop is None):
            ...
            if callable(on_stop):
                ...
                if (on_stop.__code__.co_argcount == 4):
                    self.on_stop = on_stop
                else:
                    self.valid_parameters = False
                    raise ValueError("The function assigned to the on_stop parameter must accept 4 parameters representing the instance of the genetic algorithm, a list of the fitness values of the solutions in the last population, the parameters to be written as headers in the file and the mode in which the algorithm is run.\nThe passed function named '{funcname}' accepts {argcount} argument(s).".format(funcname=on_stop.__code__.co_name, argcount=on_stop.__code__.co_argcount))
            else:
                self.valid_parameters = False
    ...
    def cal_pop_fitness(self, constargs, multiproc=False, num_proc=1, pool=None):
...
#             if __name__ == "pygad.pygad":
    ...
    def cal_pop_single_fitness(self, constargs, gene, gene_idx):        
        return self.fitness_func(ga_instance=self, constargs=constargs, turb=None, solution=gene, solution_idx=gene_idx, mode_fitness='GA', out='fitness')
    
    def run(self, constargs, multiproc=False, num_proc=1, par_to_fit='IDOM', writepops=False, rootfilename='', pars_to_write=''):
        ...
        if not (self.on_start is None):
            self.on_start(self, par_to_fit, writepops, rootfilename, pars_to_write, constargs["Mode"])
        ...
        for generation in range(self.num_generations):
            ...
            if multiproc:
                ...
                self.last_generation_fitness = self.cal_pop_fitness(constargs, multiproc=multiproc, num_proc=num_proc, pool=pool)
            else:
                self.last_generation_fitness = self.cal_pop_fitness(constargs)
            if not (self.on_fitness is None):
                self.on_fitness(self, self.last_generation_fitness)
                        
            if type(self.last_generation_fitness[0]) == numpy.ndarray:
                best_solution, best_solution_fitness, best_match_idx = self.best_solution(pop_fitness=[_[0] for _ in self.last_generation_fitness])
            else:
            ...
            if type(self.last_generation_fitness[0]) == numpy.ndarray:
                self.last_generation_parents = self.select_parents([_[0] for _ in self.last_generation_fitness], num_parents=self.num_parents_mating)
            else:
            ...
            if not (self.on_generation is None):
                r = self.on_generation(self, pars_to_write, constargs["Mode"])
        ...
        if type(self.last_generation_fitness[0]) == numpy.ndarray:
            _, best_solution_fitness, _ = self.best_solution(pop_fitness=[_[0] for _ in self.last_generation_fitness])
        else:
        ...
        if not (self.on_stop is None):
            self.on_stop(self, self.last_generation_fitness, pars_to_write, constargs["Mode"])
    ...
    def adaptive_mutation_population_fitness(self, offspring):
        ...
        for idx, sol in enumerate(temp_population):
            fitness[idx] = self.fitness_func(self, sol, None)
